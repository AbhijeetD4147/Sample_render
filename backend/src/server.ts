...
// Updated CORS policy to allow all origins for demonstration purposes.
// In production, replace '*' with specific allowed origins or use environment variable.
app.use(cors({
  origin: '*',
...
init({
  publicKey: process.env.EMAILJS_PUBLIC_KEY || "ngfcrIBDBDILAGrej",
  privateKey: process.env.EMAILJS_PRIVATE_KEY || "your_private_key", // Add your private key here
});

// Remove commented out send-otp endpoint to avoid exposing test code
